...
import { expiringDomains } from "@/data/mockDomains";
          ...
          <header className="sticky top-0 z-40 bg-background/95 backdrop-blur-xl border-b border-border safe-area-pt">
            <div className="flex items-center gap-3 px-4 py-3">
              ...
              <div>
                ...
                <p className="text-xs text-muted-foreground">{t("expiring.subtitle")}</p>
              </div>
            </div>
          </header>
        )}
            ...
            <div className="col-span-2">
              <div className="mb-6">
                <div className="flex items-center gap-3 mb-2">
                  ...
                  <div>
                    <h1 className="text-xl font-semibold text-foreground">{t("expiring.title")}</h1>
                    <p className="text-sm text-muted-foreground">{t("expiring.subtitle")}</p>
                  </div>
                </div>
              </div>
              <SectionCard className="divide-y divide-border">
                {expiringDomains.map((domain) => (
                  <DomainListItem key={domain.domain} domain={domain} showFollowButton subtitle="vibe" />
                ))}
              </SectionCard>
              {expiringDomains.length === 0 && (
                <div className="flex flex-col items-center justify-center px-6 py-20 text-center">
                  <p className="text-muted-foreground">{t("expiring.noDomains")}</p>
                </div>
              )}
            </div>
            <div className="space-y-6">
              <SectionCard className="bg-gradient-to-br from-destructive/10 to-orange-500/10 border-destructive/20">
                <div className="flex items-start gap-3">
                  <AlertTriangle className="w-5 h-5 text-destructive shrink-0 mt-0.5" />
                  <div>
                    <h4 className="text-sm font-semibold mb-1">{t("expiring.alert")}</h4>
                    <p className="text-xs text-muted-foreground leading-relaxed">{t("expiring.alertDesc")}</p>
                  </div>
                </div>
              </SectionCard>
            </div>
